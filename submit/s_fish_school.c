...
int main(int argc, char *argv[])
{
	...
	#ifdef SORT
	const char *delta_scheme = "SORT";
	#endif
	#ifdef NORMAL
	const char *delta_scheme = "NORMAL";
	#endif
	// stop the clock
	end_timer(&time_results);
	printf("sequential,1,%d,%d,%f,%f,%f,%f,,%s,\n", SCHOOL, STEPS, time_results.cpu_cycles_elapsed, time_results.omp_elapsed, time_results.monotonic_elapsed, time_results.posix_cputime_elapsed, delta_scheme);
}
